#include "tdrstyle.C"
TFile *fileStage1Unpacked, *fileStage1ReRunNOPU,*fileStage1ReRun;
...
void prettyPlottingJetsPU(){

	setTDRStyle();

	fileStage1ReRunNOPU=new TFile("outputs/DataStage1833NOPU.root","READONLY");
//	fileStage1Unpacked=new TFile("outputs/DataGCT833Unpacked.root","READONLY");
	fileStage1Unpacked=new TFile("outputs/DataStage1833DEF.root","READONLY");
        ...
        plot("recoEtaLead","recoEtaLead","Events");
        plot("recoPtLead","recoPtLead","Events");
        ...
        plot("hLeadPhiJet","Lead Jet #phi","Events");
        plot("hDeltaREleJet","Lead Jet-Electron angle","Events");
        plot("hDeltaPhiEleJet","Lead Jet-Electron angle","Events");
        plot("hDeltaEtaEleJet","Lead Jet-Electron angle","Events");
	...
	res("hResLeadPtJet2DEta","Reco #eta","(l1pt-recopt)/recopt",0.3,0.7,0.15,0.3);
        res("hResLeadPtJet2D","Reco jet Pt","(l1pt-recopt)/recopt",0.3,0.7,0.15,0.3);
        res("hResLeadPtJet2DL1","L1 jet Pt","(l1pt-recopt)/recopt",0.3,0.7,0.15,0.3);
        res("hResLeadPtJet2DNVTX","Reco Vertices","(l1pt-recopt)/recopt",0.3,0.7,0.15,0.3);
        ...
        effi("effi_nvtx_Jet70",70,"Trigger Efficiency, SingleJet70","Events",0.5,0.95,0.2,0.5);
...
void plot(TString plot, TString xlabel, TString ylabel, double xlegmin=0.2, double xlegmax=0.4, double ylegmin=0.75, double ylegmax=0.85){	
	...
	TH1F *histo=(TH1F*)fileStage1ReRunNOPU->Get(plot); histo->SetName("Stage1Unpacked_"+plot);
	TH1F *histoDefault=(TH1F*)fileStage1Unpacked->Get(plot); histoDefault->SetName("Stage1_"+plot);
	...
	histo->Draw("E,hist");
	histoDefault->Draw("sames,E,hist");
        histoReRun->Draw("sames,E,hist");
	...
	TLegendEntry *entry=leg->AddEntry(histoDefault,"Stage1, Old Corrections","p");
        entry=leg->AddEntry(histoReRun,"Stage1, New Corrections","lf");
	entry=leg->AddEntry(histo,"Stage1, New Corrections, NO PU","lf");
	leg->Draw();
	cmsPrelim(LUMI);
...
void effi(TString plot, double threshold,TString xlabel, TString ylabel, double xlegmin=0.5, double xlegmax=0.9, double ylegmin=0.2, double ylegmax=0.5){	
	...
	TH1F *histo=(TH1F*)fileStage1ReRunNOPU->Get(plot); histo->SetName("NOPU_"+plot);
	TH1F *histoDefault=(TH1F*)fileStage1Unpacked->Get(plot); histoDefault->SetName("Stage1_"+plot);
	...
	histo->Draw("hist,E");
	histoDefault->Draw("hist,sames,E");
        histoReRun->Draw("hist,sames,E");
	...
	histo->GetYaxis()->SetRangeUser(0,1.1);
        histo->GetXaxis()->SetRangeUser(0,threshold+100);

	cmsPrelim(LUMI);

        TLegend *leg = new TLegend(xlegmin,ylegmin,xlegmax,ylegmax,NULL,"brNDC");
        TLegendEntry *entry=leg->AddEntry(histoDefault,"Stage1, Old Corrections","pl");
        entry=leg->AddEntry(histoReRun,"Stage1, New Corrections","pl");
        entry=leg->AddEntry(histo,"Stage1, New Corrections, NO PU","pl");
        leg->Draw();
...
void res(TString plot, TString xlabel, TString ylabel, double xlegmin=0.4, double xlegmax=0.7, double ylegmin=0.4, double ylegmax=0.5){
        ...
        TH2F *histo2D=(TH2F*)fileStage1ReRunNOPU->Get(plot); histo2D->SetName("NOPU_2D_"+plot);
        TH2F *histoReRun2D=(TH2F*)fileStage1ReRun->Get(plot); histoReRun2D->SetName("ReRun_2D_"+plot);
        TH2F *histoDefault2D=(TH2F*)fileStage1Unpacked->Get(plot); histoDefault2D->SetName("Default_2D_"+plot);

	TH1F* histo=(TH1F*)histo2D->ProfileX(); histo->SetName("NOPU_"+plot);
	...
	histoReRun->GetYaxis()->SetRangeUser(-0.25,0.25);
	...
	entry=leg->AddEntry(histoDefault,"Stage1, Old Corrections","p");
        entry=leg->AddEntry(histoReRun,"Stage1, New Corrections","lf");
        entry=leg->AddEntry(histo,"Stage1, New Corrections, NO PU","lf");
        leg->Draw();
