#include "tdrstyle.C"
TFile *file25nsDEF, *file25nsReRun;
...
void prettyPlotting(){
	...
	file25nsReRun=new TFile("outputs/DataStage1833.root","READONLY");
	file25nsDEF=new TFile("outputs/DataStage1833DEF.root","READONLY");
...
void plot(TString plot, TString xlabel, TString ylabel, double xlegmin=0.2, double xlegmax=0.4, double ylegmin=0.75, double ylegmax=0.85){	
	...
	TH1F *histo=(TH1F*)file25nsReRun->Get(plot); histo->SetName("25nsDEF_"+plot);
	TH1F *histoDefault=(TH1F*)file25nsDEF->Get(plot); histoDefault->SetName("Stage1_"+plot);

	histo->SetMarkerColor(kOrange);
        histo->SetLineColor(kOrange+4); 
	histo->SetMarkerStyle(21);
	histoDefault->SetMarkerStyle(20);
	...
	TLegendEntry *entry=leg->AddEntry(histoDefault,"Default","p");
	entry=leg->AddEntry(histo,"ReRun","lf");
	leg->Draw();
	cmsPrelim(LUMI);
...
void effi(TString plot, TString xlabel, TString ylabel, double xlegmin=0.5, double xlegmax=0.7, double ylegmin=0.3, double ylegmax=0.5){	
	...
	histo->SetLineColor(kOrange+2);  histoDefault->SetMarkerStyle(25); histo->SetMarkerColor(kOrange+2); histo->SetLineWidth(2);
	histoDefault->SetMarkerStyle(20); histoDefault->SetLineColor(kBlack);  histoDefault->SetLineWidth(2);
